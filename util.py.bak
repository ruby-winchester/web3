...
def word(student_st_id,exam_ex_id,filepath):
    ...
    document.add_heading('\t\t' + exam['ex_name'], 0)
    ...
    # )
    document.add_heading('选择题', 1)
    for item in choice_question:
        # print item
        if item['eq_get_score'] < 0:
            # item['eq_get_score'] = "未出分"
            document.add_paragraph(
                item['qt_stem'] + '\n 学生答案: ' + item['eq_answer'] + '\t 得分: ' + '未出分',
                style='List Number')
        else:
            document.add_paragraph(
                item['qt_stem'] + '\n 学生答案: ' + item['eq_answer'] + '\t 得分: ' + str(item['eq_get_score']),
                style='List Number')

        document.add_paragraph(
            'A. ' + item['cc_a'], style='List Bullet'
        )
        document.add_paragraph(
            'B. ' + item['cc_b'], style='List Bullet'
        )
        document.add_paragraph(
            'C. ' + item['cc_c'], style='List Bullet'
        )
        document.add_paragraph(
            'D. ' + item['cc_d'], style='List Bullet'
        )

    document.add_heading('判断题', 1)
    ...
    for item in fillb_question:
        ...
        for eq in item[1]:
            if eq['eq_get_score'] < 0:
                ...
                answer += '空' + str(i) + '\t' + eq.eq_answer + '\t 得分: ' + "未出分" + '\n'
            else:
                answer += '空' + str(i) + '\t' + eq.eq_answer + '\t 得分: ' + str(eq['eq_get_score']) + '\n'
            i += 1
        document.add_paragraph(
            item[0]['qt_stem'] + '\n' + item[0]['fb_pre_coding'] + '\n' + answer, style='List Number'
        )

    document.add_heading('编程题', 1)
...
def QuestionWord(question_list,question_back_id,filepath):
    ...
    document.add_heading('\t\t' + questionback.qb_name, 0)
    choice_question = []
    judge_question = []
    # filla是读程序写结果
    filla_question = []
